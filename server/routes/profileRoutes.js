...
router.get('/:handle', protect, async (req, res) => {
  try {
    ...
    // Get Authored Posts
    const authoredPosts = await Post.find({ identity: identity._id, visibility: 'public' })
      .sort({ createdAt: -1 })
    ...
    const repostedPosts = await Post.find({ 'reposts.identity': identity._id, visibility: 'public' })
      .sort({ createdAt: -1 })
      .populate('identity', 'name type handle avatar')
      .populate('reposts.identity', 'name type handle avatar');
